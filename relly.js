...
//------------------------------------------------------------------------------------------------------------\\
  ...
  client.on("message", message => {
    ...
    return message.channel.send(("Criés  & #1901"))
});
...
client.on("message", async message => {
  ...
  client.on("userUpdate", async function(oldUser, newUser) {
    const guildID = "848270812436103218"//sunucu
    const roleID = "857672499159105536"//taglırolü
    const tag = "Criés "//tag
    const chat = '848662315150999593'// chat
    const log2 = '857222688086294558' // log kanalı
   ...
   if (newUser.discriminator !== oldUser.discriminator) {
        if (oldUser.discriminator == "1901" && newUser.discriminator !== "1901") {
            member.roles.remove(roleID)
            client.channels.cache.get(log2).send(embed.setDescription(`  ${newUser} etiketinden \`1901\` çıakrtarak ailemizden ayrıldı!`))
        } else if (oldUser.discriminator !== "1901" && newUser.discriminator == "1901") {
            member.roles.add(roleID)
...
client.on("guildMemberAdd", member => {
  const lorewebhook = new Discord.WebhookClient('861034993903206420', '6dkzvZSDjHuU3aaDNjaaFLh5gQQmei-8PFrPHbLB3R50FBNoc6GtuJX-pXAj45M0w0Pg')

  let sunucuid = "848270812436103218"; 
  var tag = 'Criés';
  let rol = "857672499159105536";
...
const mutelogs = new Discord.WebhookClient(config.mutelogid, config.mutelogtoken)
  ...
  if(muteData) {
      member.roles.add("848889131135926282") //Muted ID
...
client.on("guildMemberAdd", member => {
  member.roles.add('848679403713724418'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
//-----------------------GİRENE-ROL-VERME----------------------\\     
...
client.on("message", async message => {
  ...
  client.channels.cache.get('816599041311899669').send(new Discord.MessageEmbed().setAuthor("Bir Dm Geldi", client.user.avatarURL()).setFooter(message.author.tag, message.author.avatarURL()).setDescription(`**Gönderenin ID:** ${message.author.id}`).setTimestamp().addField("Mesaj", message.content).setColor("RANDOM"))
})
//------------------------------------------------------------------------------------------------------------\\ 
// spoengel
